...
- `src/sdologin/account_history_combo.h/.cpp`
  ...
  - Delete button first closes the popup and updates the in-memory row list, then queues the outer `userinfo.xml` write to the next Qt event tick so the clicked button never re-enters config write or combo refresh while its own click event is still executing.
...
- The deleted row is removed from the next dropdown open without synchronously rebuilding controls or writing config during the remove-button click handler.
