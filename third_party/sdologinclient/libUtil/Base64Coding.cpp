...
const string CBase64Coding::Base64Table("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/");
...
bool CBase64Coding::Encode(const char* souce,int srclen,char* dest,int destlen)
{
	string str = Encode(souce,srclen);
	if(str.size() > destlen) return false;
	memcpy_s(dest,destlen,str.c_str(),str.size());
	return true;
}

string CBase64Coding::Decode(const string& data)
{
	string::size_type  i;
	char               c;
	char               c1;
...
int CBase64Coding::Decode(const string& data,char* dest,int destlen)
{
	string::size_type  i;
	char               c;
	char               c1;
	...
	int retlen = 0;
	if(NULL == dest || destlen < len) return -1;
